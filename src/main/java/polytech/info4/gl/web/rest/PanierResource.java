...
import polytech.info4.gl.repository.PanierRepository;
import polytech.info4.gl.web.rest.errors.BadRequestAlertException;
import tech.jhipster.web.util.HeaderUtil;
import tech.jhipster.web.util.ResponseUtil;

/**
 * REST controller for managing {@link polytech.info4.gl.domain.Panier}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PanierResource {
    ...
    public ResponseEntity<Panier> partialUpdatePanier(
        ...
        Optional<Panier> result = panierRepository
            ...
            .map(existingPanier -> {
                ...
                if (panier.getPaiement() != null) {
                    existingPanier.setPaiement(panier.getPaiement());
                }
    ...
    public List<Panier> getAllPaniers(@RequestParam(required = false) String filter) {
        if ("livraison-is-null".equals(filter)) {
            log.debug("REST request to get all Paniers where livraison is null");
            return StreamSupport
                .stream(panierRepository.findAll().spliterator(), false)
                .filter(panier -> panier.getLivraison() == null)
